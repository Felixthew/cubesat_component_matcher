import pandas as pd
from src.backend_solution.json_types import SearchKwargs
from src.backend_solution.scorer import SCORING_REGISTRY, SCORING_CONFIG
...
class ScoringEngine:
    ...
    def _score_single(self, column_name, request_val, candidate_val):
        ...
        # determine column datatype and therefore which scorer to use
        dtype = self.dtypes[column_name]
        scorer = SCORING_REGISTRY[dtype]

        # initialize kwarg dict
        all_kwargs = {}

        # min and max vals need to be passed through configs for numbers
        if dtype == "number":
