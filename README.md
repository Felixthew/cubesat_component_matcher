...
If your pip install completed with no errors yet the script can't find the libraries. I run into this problem all the time and it comes down to the fact that the pip is installing to a different
location the where the script is checking for libraries. One easy fix if you use pycharm is to pip install from the ide window with your project.
